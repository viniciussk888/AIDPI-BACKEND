  ...
  async validStep({ request, response }) {
    ...
    async function step2(data) {
      if (data.tiragemSubcostal == true && data.sinalDeRisco == true && data.estridor == true) {
        response.send({
          diagnostico: "PNEUMONIA GRAVE OU DOENÇA MUITO GRAVE",
          tratamento: "• Dar a primeira dose de um antibiótico recomendado • Tratar a criança para evitar hipoglicemia • Referir urgentemente ao hospital • Oxigênio, se disponível"
        })
      }
      else if (data.respiraçaoRapida == true && data.tiragemSubcostal == false && data.sinalDeRisco == false && data.estridor == false) {
        response.send({
          diagnostico: "PNEUMONIA",
          tratamento: "Dar um antibiótico recomendado durante sete dias • Aliviar a tosse com medidas caseiras • Informar à mãe sobre quando retornar imediatamente • Marcar o retorno em dois dias."
        })
      }
      else {
        response.send({
          diagnostico: "NÃO É PNEUMONIA, possivel resfriado comum",
          tratamento: "Aliviar a tosse com medidas caseiras • Informar a mãe sobre quando retornar imediatamente • Seguimento em cinco dias, se não melhorar • Se tosse há mais de 14 dias , realizar investigação."
        })
      }
